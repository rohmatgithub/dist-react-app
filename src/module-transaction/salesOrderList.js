...
export default function SalesOrderList() {
  ...
  const fetchListData = async (page, strSearch) => {
    const response = await ApiGet(
      `${uriTrans}/order?page=${page}&order_by=updated_at DESC${strSearch}`,
      auth.token
    );
  ...
  const fetchCountData = async (strSearch) => {
    const response = await ApiGet(
      uriTrans + `/order/count${strSearch}`,
      auth.token
    );
  ...
  const handleSearch = (event) => {
    ...
    let strSearch = "";
    if (valueSearch !== "") {
      strSearch = `filter=order_number like ${valueSearch}`;
    }
    ...
    <div>
              ...
              data.map((d, i) => {
                return (
                  <tr class="bg-white border-b hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 font-medium text-black">
                      {formatRupiah(d.total_gross_amount)}
                    </td>
                    <td class="px-6 py-4 font-medium text-black">
                      {formatRupiah(d.total_net_amount)}
                    </td>
                    <td class="flex px-6 py-4 font-medium text-black">
                      {d.is_generated ? (
                      ...
                      <div
                        className={
                          d.is_generated
                            ? "ml-4 w-3 h-3 rounded-full bg-green-500"
                            : "ml-4 w-3 h-3 rounded-full bg-red-500"
                        }
                      />
                    </td>
                  </tr>
      ...
      <div className="mt-4 flex items-center">
        ...
        <div className="ml-2 text-gray-500">
          : Sales Order sudah diterbitkan menjadi Invoice
        </div>
      </div>
      <div className="flex items-center">
        <div className="w-3 h-3 rounded-full bg-red-500" />
        <div className="ml-2 text-gray-500">
          : Sales Order belum diterbitkan menjadi Invoice
        </div>
      </div>
    </div>
  );

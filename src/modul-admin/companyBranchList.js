...
import { AlertCustomStyles, notifyError } from "../components/alert";
...
export default function CompanyBranchList() {
  ...
  const fetchCountData = async () => {
    const response = await ApiGet(
      uriMaster + "/companybranch/initiate",
      auth.token
    );
  ...
  const buttonAdd = (event) => {
    ...
    navigate("/admin/companybranch/add");
  };
            ...
            data.map((d, i) => {
              return (
                <tr class="bg-white border-b hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 font-medium text-black">
                    <Link
                      to={`/admin/companybranch/edit/${d.id}`}
                      class="font-medium text-blue-600 hover:underline"
                    >
                      <FontAwesomeIcon icon={faEdit} />
                    </Link>
                    <Link
                      to={`/admin/companybranch/detail/${d.id}`}
